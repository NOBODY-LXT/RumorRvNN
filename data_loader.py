...
class RumorDataLoader():
    ...
    def processing(self, dataset, vocab, idf):
        ...
        for eid in trees:
            ...
            all_adj[eid] = torch.from_numpy(adj).float().to(self.device)
            all_tfidf[eid] = torch.from_numpy(tfidf_matrix).float().to(self.device)
            all_labels[eid] = torch.from_numpy(np.array(label_mapping[eid_label])).long().unsqueeze(0).to(self.device)
            all_level[eid] = torch.from_numpy(np.array(level)).float().to(self.device)
